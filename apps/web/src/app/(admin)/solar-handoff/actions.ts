  ...
  try {
    ...
    const dataRows = selected.map((r) => {
      ...
      return [
        ...
        r.name,
        r.kana ?? '',
        r.phoneEnc instanceof Uint8Array ? Buffer.from(r.phoneEnc).toString('utf8') : r.phoneEnc,
        r.emailEnc instanceof Uint8Array ? Buffer.from(r.emailEnc).toString('utf8') : (r.emailEnc ?? ''),
        r.birthDate ?? '',
        r.postalCode ?? '',
        r.prefecture ?? '',
        r.city ?? '',
